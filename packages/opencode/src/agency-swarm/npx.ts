...
async function installProjectDependencies(
  ...
  const commandOptions = {
    ...
    streamOutputToStderr: Boolean(options?.logFile),
    timeoutMs: options?.timeoutMs,
  }
  const requirements = path.join(directory, "requirements.txt")
  if (await Filesystem.exists(requirements)) {
    const result = uv
      ? await runCommand([uv, "pip", "install", "--python", python[0], "-r", "requirements.txt"], commandOptions)
      : await runCommand([...python, "-m", "pip", "install", "--upgrade", "-r", "requirements.txt"], commandOptions)
  ...
  if (await Filesystem.exists(pyproject)) {
    const result = uv
      ? await runCommand([uv, "pip", "install", "--python", python[0], "-e", "."], commandOptions)
      : await runCommand([...python, "-m", "pip", "install", "--upgrade", "-e", "."], commandOptions)
    return { ...result, hadManifests: true }
  }

  const result = uv
    ? await runCommand(
        [uv, "pip", "install", "--python", python[0], "agency-swarm[fastapi,litellm]>=1.9.5"],
        commandOptions,
      )
    : await runCommand(
...
async function venvCanaryPasses(python: string[], options?: { cwd?: string; includeStderr?: boolean }) {
  ...
  // sitting next to `swarm.py`) that shadow installed packages and falsely flag a healthy
