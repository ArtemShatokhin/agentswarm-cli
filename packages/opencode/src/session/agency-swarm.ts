...
export namespace SessionAgencySwarm {
  ...
  function historyMissingLocalUserMessages(
    ...
    const storedUserTexts = new Set(stored.map(historyUserText).filter(Boolean))
    return rebuilt.some((item) => {
      const text = historyUserText(item)
      return !!text && !storedUserTexts.has(text)
    })
  }
  ...
  function toolPartToHistoryItems(
    ...
    const metadata =
      state.status === "completed" || state.status === "error" ? asRecord(state.metadata) : undefined
    const base = {
      agent: msg.info.agent,
      callerAgent,
      timestamp: msg.info.time.created,
