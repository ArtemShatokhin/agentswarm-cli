...
- `USER_FLOWS.md` Detected Local Project: launcher mode shows startup project-file checking, the detected-project choice before `.venv` work begins, unreadable `agency.py` recovery choices, startup import failure fallback into Build with the error prefilled, same-project Run startup after switching back to Run without stale env server config reuse, external `/connect` from that fallback, and Agent Swarm connect copy.
